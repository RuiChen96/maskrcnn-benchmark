...
def main():
    ...
    cat_file = "/Users/ruichen/Data/categories_shanghai_30skus.txt"
    cat_name_2_id = {}
    with open(file=cat_file, mode='r') as fp:
        cat_names = fp.readlines()
    for cat_name in enumerate(cat_names):
        name = cat_name[1].strip()
        id = cat_name[0]
        if name != 'BACKGROUND':
            cat_name_2_id[name] = id

    sub_folders = os.listdir(img_dir)
    for sub_folder in sub_folders:
        current_label = sub_folder
        sub_folder_2_id = cat_name_2_id[current_label]
        # TODO
        current_dir = os.path.join(img_dir, sub_folder)
        ...
        for img in imgs:
            ...
            file_name = img
            img_cv2 = cv2.imread(os.path.join(current_dir, file_name))
